...
def collate_MIL_survival_path(batch):
    img = torch.cat([item[0] for item in batch], dim = 0)
...
def collate_MIL_survival_omic(batch):
...
def collate_MIL_survival_bert_coattn(batch):
    img = torch.cat([item[0] for item in batch], dim = 0)
    omic = torch.cat([item[1] for item in batch], dim = 0)
...
def get_split_loader(split_dataset, training = False, testing = False, weighted = False, modal='coattn', batch_size=1):
    ...
    if modal == 'coattn':
        ...
        collate = collate_MIL_survival_path
    elif modal == 'omic':
        collate = collate_MIL_survival_omic
    elif modal == 'bert_coattn':   
        collate = collate_MIL_survival_bert_coattn
