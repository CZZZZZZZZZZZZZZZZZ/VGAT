...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self,
        ...
        if self.apply_sig:
            self.signatures = pd.read_csv('VGAT-main/csv/signatures.csv')
        else:
    ...
    def get_split_from_df(self, all_splits: dict, split_key: str = 'train', scaler=None):
        ...
        if len(split) > 0:
            ...
            split = Generic_Split(df_slice, metadata=self.metadata, modal=self.modal, signatures=self.signatures,
                                  data_dir=self.data_dir,Bert_data_dir = self.Bert_data_dir,label_col=self.label_col, patient_dict=self.patient_dict, num_classes=self.num_classes,OOM = self.OOM)
        else:
            split = None

        return split

    def return_splits(self, from_id: bool = True, csv_path: str = None,Normalizing_Data = True):
        if from_id:
...
class Generic_MIL_Survival_Dataset(Generic_WSI_Survival_Dataset):
    def __init__(self, data_dir,Bert_data_dir,OOM,modal = 'omic', **kwargs):
        super(Generic_MIL_Survival_Dataset, self).__init__(**kwargs)
        self.data_dir = data_dir
        self.Bert_data_dir = Bert_data_dir
        self.modal = modal
        self.use_h5 = False
        self.OOM = OOM
    ...
    def __getitem__(self, idx):
        ...
        #if self.Rna_dir is not None:
        #    rna_id = self.slide_data['rna_id'][idx]
        ...
        if type(self.data_dir) == dict:
            ...
            if not self.use_h5:
                if self.modal == 'path':
                    ...
                    with warnings.catch_warnings():
                        ...
                        for slide_id in slide_ids:
                            ...
                            if os.path.exists(wsi_path):
                                ...
                                print(f"Warning: File {wsi_path} does not exist. Skipping...")
                
                        if len(path_features) == 0:
                            print("No valid '.pt' files found in the specified data directory.",case_id)
                            raise ValueError("No valid '.pt' files found in the specified data directory.")
                    ...
                    return (omic1, omic2, omic3, omic4, omic5, omic6, label, event_time, c)
                
                elif self.modal == 'bert_coattn':
                    rna_id = self.slide_data['rna_id'][idx]
                    omic_path = os.path.join(self.Bert_data_dir, rna_id)
                    omic_features = torch.load(omic_path) 
                    omic_features = torch.tensor(omic_features)
                    import warnings
                    with warnings.catch_warnings():
                        warnings.simplefilter('ignore', FutureWarning)
                        path_features = []
...
class Generic_Split(Generic_MIL_Survival_Dataset):
    def __init__(self, slide_data, metadata, modal, signatures=None, data_dir=None,OOM = 0, Bert_data_dir = None,label_col=None, patient_dict=None, num_classes=2):
        self.use_h5 = False
        self.slide_data = slide_data
